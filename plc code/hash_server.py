...
def send_to_elasticsearch(hash2, hash3, match_status):
    ...
    es.index(index=f'hash-{current_time}', body={"hash_host2": hash2, "hash_host3": hash3, "match_status": match_status, 'timestamp': datetime_kst.isoformat()})
...
def listen_hashes():
    while True:
        ...
        if len(hashes_received_host2) > 0 or len(hashes_received_host3) > 0:
                ...
                if(len(hashes_received_host2) > len(hashes_received_host3)):
                    ...
                    if(len(hashes_received_host3)!=0):
                        ...
                        if(match_status=="Mismatch"):
                            es_thread = threading.Thread(target=send_to_elasticsearch, args=(hashes_received_host2[0], hashes_received_host3[0], "Unknown Hash"))
                            es_thread.start()
                        ...
                        es_thread = threading.Thread(target=send_to_elasticsearch, args=(hashes_received_host2[0], "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", "Unknown Hash"))
                        es_thread.start()
                        hashes_received_host2.pop(0)

                elif(len(hashes_received_host2) < len(hashes_received_host3)):
                    ...
                    if(len(hashes_received_host2)!=0):
                        ...
                        es_thread = threading.Thread(target=send_to_elasticsearch, args=( "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",hashes_received_host3[0], "Unknown Hash"))
                        es_thread.start()
                        hashes_received_host3.pop(0)
